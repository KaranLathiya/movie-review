...
func (r *movieResolver) Reviews(ctx context.Context, obj *model.Movie, limit int, offset int) ([]*model.MovieReview, error) {
	...
	movieReviewLimit := response.MovieReviewLimit{ID: obj.ID, Limit: limit, Offset: offset}
	reviews, err := ctx.Value(constant.LoaderCtxKey).(*dataloader.Loaders).ReviewLimitLoader.Load(movieReviewLimit)
	return reviews, err
}
